#!/usr/bin/env bash
#
# mayhem/test.sh — RUN gitignore_parser's own test suite (tests.py, unittest-style, run via pytest;
# deps already installed by mayhem/build.sh) and emit a CTRF (ctrf.io) summary. exit 0 iff
# failed==0. PATCH-grade oracle: a no-op patch that neuters the library breaks the suite's
# behavioral assertions, so it FAILS here (anti-reward-hacking).
#
# It does NOT compile — build.sh installed pytest into the in-image site dir and compiled the
# gitignore_run_tests ELF wrapper. We only RUN the suite. The suite is routed through that compiled
...
# Put the in-image site dir (atheris + pytest) + the repo root (gitignore_parser module) on PYTHONPATH.
...
RUNNER="$SRC/gitignore_run_tests"
if [ ! -x "$RUNNER" ]; then
...
# Run the suite. -p no:cacheprovider keeps the read-only image happy; tests.py is the project's suite.
LOG="$(mktemp)"
"$RUNNER" -p no:cacheprovider -o addopts= -q tests.py 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}
