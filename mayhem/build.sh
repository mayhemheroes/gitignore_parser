#!/usr/bin/env bash
#
# mayhem/build.sh — build the gitignore_parser Atheris fuzz harness + its standalone reproducer,
# and prepare the project's own unittest suite (run via pytest). Runs inside the commit image
...
#      resolves entirely from it (pip --no-index --find-links). gitignore_parser itself stays the
#      editable source tree at the repo root (on PYTHONPATH) so a PATCH agent's edits take effect.
#   2. Compile launcher.c -> the ELF Mayhem target `gitignore_fuzzer` (Atheris is a Python
#      script; Mayhem needs an ELF cmd, and the gate needs DWARF < 4 — hence a compiled wrapper).
#   3. Build the same launcher as the standalone (run-once) reproducer `gitignore_fuzzer-standalone`.
#   4. Compile run_tests.c -> the ELF wrapper that runs the pytest suite (so the gate's sabotage
#      check bites — see run_tests.c).
#
...
#    manylinux wheel for this CPython, so no compilation is needed. gitignore_parser has NO runtime
#    deps (pure stdlib); pytest runs its unittest-style suite.
PKGS=(atheris pytest)
need_download=0
...
# gitignore_parser itself: keep it as the editable source tree (the single module at the repo root)
# so a PATCH agent's edits to gitignore_parser.py take effect immediately. Expose it by putting
# $SRC (the repo root) on PYTHONPATH — both via the baked ENV in the Dockerfile (run time) and via
# env.sh / the explicit sanity check below (build time).
PYRUN="$SITE:$SRC"
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, gitignore_parser, pytest; print("imports OK:", gitignore_parser.__file__)'
...
HARNESS="$SRC/mayhem/fuzz_gitignore.py"
echo ">> compiling gitignore_fuzzer (+ standalone) with DEBUG_FLAGS=$DEBUG_FLAGS"
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/gitignore_fuzzer"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/gitignore_fuzzer-standalone"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" "$SRC/mayhem/run_tests.c" -o "$SRC/gitignore_run_tests"

echo ">> build.sh complete"
ls -la "$SRC/gitignore_fuzzer" "$SRC/gitignore_fuzzer-standalone" "$SRC/gitignore_run_tests"
